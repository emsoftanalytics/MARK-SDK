...
#
# SafetyConstraintLayer — governance-invariant filter for learned policy actions.
...
class RLAction:
    """
    Structured action returned by a learned policy plugin (HOOK_RL_POLICY).

    Open-contract note:
    This Apache-2.0 SDK type is only the public action envelope. The policy model,
    training pipeline, model registry, inference, and adaptation logic are not
...
class SafetyConstraintLayer:
    ...
    Open-contract note:
    This Apache-2.0 SDK class is an auditable safety layer for actions produced by
    a registered policy plugin. It does not implement the policy, reinforcement
    learning model, online finetuner, or model-version registry.
    ...
    The learned policy plugin (HOOK_RL_POLICY) is constrained by this layer
    before actions are applied to the developer's runtime.
    """
